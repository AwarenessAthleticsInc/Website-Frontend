...
const RegistrationGrowthChart = (props) => {
    ...
    const yearsArray = useSelector(state => state.metrics.registrationGrowth);
    useEffect(() => {
    ...
    return <Paper sx={{ p: '0.8rem', display: 'flex', flexDirection: 'column', height: 'auto', aspectRatio: '8/5.15' }}>
        ...
        {currentYear === 2022 && <Typography variant="p" gutterBottom>{`${years[0][12] === undefined ? "0" : years[0][12].length} registrations aren't tracked on the chart above`}</Typography>}
    </Paper>
}

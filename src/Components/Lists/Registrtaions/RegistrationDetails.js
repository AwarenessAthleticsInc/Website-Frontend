...
const RegistrationDetails = (props) => {
    ...
    const update = () => {
        Axios.put('/api/dashboard/registrations', {
            id: registrations[index]._id,
            team: team 
        }).then((response) => {
            const indexOfSelected = registrationArray.findIndex((reg) => {
                return reg._id === response.data._id;
            });
            const newArray = replace(indexOfSelected, registrationArray, response.data);

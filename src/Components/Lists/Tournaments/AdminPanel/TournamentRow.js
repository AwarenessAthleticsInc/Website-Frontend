...
const TournamentRow = (props) => {
    ...
    return <React.Fragment>
        ...
        >
            ...
            <TableCell key={`date-${row._id}`} align={props.align || 'center'}>{`${new Date(row.start.date).toLocaleDateString('en-us')}`}</TableCell>
            <TableCell key={`location-${row._id}`} align={props.align || 'center'}>{`${row.city}, ${row.diamond}`}</TableCell>
            <TableCell key={`registered-${row._id}`} align={props.align || 'center'}>{`${registrations.length}`}</TableCell>
            <TableCell key={`left-${row._id}`} align={props.align || 'center'}>{`${row.spots}`}</TableCell>
            ...
            <TableCell key={`register-${row._id}`} align={props.align || 'center'} sx={{maxWidth: '150px'}}>
                <RegisterButton key={`registerButton-${row._id}`} registrations={registrations} tournament={row} count={row.spots} onClick={register} />
            </TableCell>
